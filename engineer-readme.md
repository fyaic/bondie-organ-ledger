...
> 第一次使用建议先看 [`OrganLedger 新手村手册`](docs/newbie-guide.md)：用截图走一遍看板 / 日志 / 文件树分别解决什么问题。
...
新用户最容易先从「日志」和「文件树」理解 OrganLedger：前者回答“哪天发生了什么”，后者回答“改动集中在哪里”。
...
- **风险默认只做提示**：高严重度或删除类变更会在看板中凸显，用户回到本机工具中进一步判断。
...
工程化的看板信息密度较高，对**非工程师用户可读性不够**。1.7 加两个面向直觉的可视化，**两种不同的隐私姿态**：
...
- **看板**：顶部 KPI（改动数 / 涉及文件 / 严重度 / 系统分布）+ 近期日报 + 器官来源面板；筛选（近7天·今日·全部 / 系统 / 严重度 / 关键字 / 来源 / 主使）；亮暗双模。
