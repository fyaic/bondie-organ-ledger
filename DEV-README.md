...
# 每次实测：重算热力图 + 起看板（三视图：看板 / 日志 / 热力图）
organledger heatmap   --home ~/.organledger-demo --full-tree
...
- **隐私（头号）**：`state/heatmap.json` 与 `/api/activity` **绝不含**文件内容 / diff / hash / `reason` / 密钥。
  - 热力图**无任何**"看文件/看 diff/打开"入口，点击最多弹计数 tooltip。
  - 敏感路径默认**打码**（名字→`•••`，`redacted:true`）但**保留** `change_count`（看得到热度、看不到是什么）。
  - `test/heatmap.test.ts` 有 2 条隐私断言（字段白名单 + 打码留热度），改动别让它们变红。
- **看板只读**：`src/dashboard/` 里**不得**出现 `execFileSync` / `child_process` / `git` / `readdirSync(target)`。
  - 目录遍历**只在 CLI 命令**（`organledger heatmap`，且只 `readdirSync` 拿名字/类型，**绝不** `readFileSync` target 内容）；看板只 `fs.readFile` state/*.json。
  - 自证：`grep -rE 'execFileSync|child_process|"git"' src/dashboard/` 应为空。
- **有界**：full-tree 遍历尊重 `config.ignore` + 硬排除 `node_modules/.git/.venv/venv/__pycache__` + `MAX_NODES=5000 / MAX_DEPTH=6 / MAX_CHILDREN=200`，超限折叠并标 `truncated`（no silent caps）。
