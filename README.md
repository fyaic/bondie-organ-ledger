...
organledger heatmap [--window all|Nd] [--full-tree] [--redact <glob,...>] [--json]  # 隐私目录热力图(颜色=频率) → state/heatmap.json（只读，绝不读文件内容）
...
- **日志 / 热力图**：见上方「活动日志 + 隐私热力图（Phase 1.7）」——白话按天日志 + 隐私 treemap（颜色=频率、无下钻）。
...
├── onboard/  init detect migrate logger doctor lifecycle autostart backfill provenance heatmap  # Phase 1.5 + 1.6(provenance) + 1.7(heatmap)
├── dashboard/  server data activity heatmap-read public/(index.html dashboard.css dashboard.js)  # 本地只读看板（来源面板 + 日志视图 + 隐私热力图）
...
```bash
node --test test/*.test.ts        # 56 个：核心 + classifier + hermes 跨语言 + onboarding(迁移/logger/paths v2/回填) + dashboard(列映射/KPI/筛选) + provenance(多源扫描/来源注入/加法式链/reflog) + activity(按天聚合/白话/上游) + heatmap(频率/有界/**隐私断言**)
...
- `heatmap.test`：changed-only 频率(叶子/目录聚合)、window 过滤、有界 full-tree(排除 node_modules/.git + 巨目录折叠 truncated + 深度封顶)、缺失 target 安全 no-op、**隐私断言①字段白名单(无内容/diff/hash/reason/密钥)**、**隐私断言②敏感 glob 打码留热度**
