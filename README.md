...
organledger backfill [--full-history] [--since-days N]  # 回放 target git 历史入账（幂等）
...
```bash
node --test test/*.test.ts        # 32 个：核心 + classifier + hermes 跨语言 + onboarding(迁移/logger/paths v2/回填) + dashboard(列映射/KPI/筛选)
